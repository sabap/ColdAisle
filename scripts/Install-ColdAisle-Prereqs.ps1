...
.DESCRIPTION
    ...
    - Deploys ColdAisle files to the IIS site physical path (default: C:\inetpub\wwwroot\ColdAisle)
...
.PARAMETER SitePhysicalPath
    IIS site root for ColdAisle. Default: C:\inetpub\wwwroot\ColdAisle.
...
function Install-Php {
    ...
    Write-Ok 'php.ini configured for IIS / ColdAisle'
...
function Install-PhpSqlsrvDrivers {
    ...
    if (-not $downloaded) {
        Write-Warn 'Could not auto-install pdo_sqlsrv. ODBC (pdo_odbc) is enough for ColdAisle.'
...
function Deploy-ColdAisleApp {
    ...
    if ($SkipDeploy) {
        ...
        if (-not (Test-ColdAisleRoot $SitePhysicalPath)) {
            Write-Warn "Site path has no setup.php/index.php yet: $SitePhysicalPath"
            Write-Warn 'Copy ColdAisle files there, or re-run without -SkipDeploy.'
        }
        return
    }

    Write-Step "Deploying ColdAisle to $SitePhysicalPath"

    $source = Get-ColdAisleDeploySource
    if (-not (Test-ColdAisleRoot $source)) {
        throw @"
Deploy source does not look like a ColdAisle root (missing setup.php / index.php):
  $source
Pass -DeploySource path\to\ColdAisle or run this script from the project scripts folder.
"@
    }
    ...
    $alreadyDeployed = Test-ColdAisleRoot $SitePhysicalPath
    if ($alreadyDeployed -and -not $Force) {
        Write-Ok "ColdAisle already present at $SitePhysicalPath (use -Force to refresh files)"
        return
    }
    ...
    if (Test-Path $configFile) {
        $configBackup = Join-Path $env:TEMP ("ColdAisle-config-backup-{0}.php" -f [guid]::NewGuid().ToString('N'))
    ...
    Write-Ok "ColdAisle ready at $SitePhysicalPath"
}
